...
angular.module('greencity.common.directives', [])
...
.directive('bg', function() {
	return {
		...
		link: function(scope, element, attr, multiBgController:any) {
...
.directive('spinnerOnLoad', function() {
	return {
		...
		link: function(scope, element, attr, preImgController:any) {
			element.on('load', function() {
		...
		link: function(scope, element, attr, ctrls) {
			...
			$timeout(function(){
				...
				element.on('click', function() {
					...
					(ionic as any).requestAnimationFrame(function() {
						$itemContent[0].style[(ionic as any).CSS.TRANSFORM] = "translate3d(-" + options_width + "px, 0px, 0px)";
				...
				if(anchors.length > 0)
				{
					angular.forEach(anchors, function(a) {
						...
						anchor.bind('click', function (event) {
							...
							var href = (event as any).currentTarget.href;
							var	options = {};
